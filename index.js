...
(() => {
    ...
    function initialize() {
        ...
        panel.append(el('h3', '', 'PlayerMode 0.6.3'), el('p', '', '每个原生账户分别配置退出密码、开放角色与模型/世界书；登录密码由酒馆管理。'), settingsButton, button('开放角色与默认角色', () => { if (!configured) { notify('请先设置退出密码。'); return; } showPassword('roles'); }), button('原生账户管理', () => { const node = document.querySelector('#admin_button'); if (host.account().admin && node) node.click(); else notify('此操作需要原生管理员账户。'); }));
        ...
        console.log('[PlayerMode] Initialized 0.6.3');
    }
