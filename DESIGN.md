...
## History ordering (0.6.3)
History ordering reuses the shelf's collection-filter buttons in a labelled group with aria-pressed selection. The three modes are recent, oldest and longest by message count. Filter first, sort a copy, then paginate; query/order changes reset paging. Per-role order stays in page memory and never changes native history or shared settings. The existing metadata line now includes a locally formatted timestamp, with explicit unknown-time fallback. No new palette or ornamental container is introduced; 44px targets wrap on narrow screens. history.js owns timestamp parsing, count normalization and stable presentation ordering. User requested publishing without further tests; this iteration has no dedicated runtime/browser verification.
